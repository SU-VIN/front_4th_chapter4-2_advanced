...
const memoizedFetch = (key: string, fetcher: () => Promise<Lecture[]>) => {
  if (apiCache.has(key)) {
...
const fetchMajors = async () =>
  (await axios.get<Lecture[]>("/schedules-majors.json")).data;
const fetchLiberalArts = async () =>
  (await axios.get<Lecture[]>("/schedules-liberal-arts.json")).data;

// TODO: 이 코드를 개선해서 API 호출을 최소화 해보세요 + Promise.all이 현재 잘못 사용되고 있습니다. 같이 개선해주세요.

const fetchAllLectures = async () => {
  console.log("API Call 1", performance.now());
  const majorsPromise1 = memoizedFetch("majors", fetchMajors);

  console.log("API Call 2", performance.now());
  const liberalArtsPromise1 = memoizedFetch("liberalArts", fetchLiberalArts);

  console.log("API Call 3", performance.now());
  const majorsPromise2 = memoizedFetch("majors", fetchMajors);

  console.log("API Call 4", performance.now());
  const liberalArtsPromise2 = memoizedFetch("liberalArts", fetchLiberalArts);

  console.log("API Call 5", performance.now());
  const majorsPromise3 = memoizedFetch("majors", fetchMajors);

  console.log("API Call 6", performance.now());
  const liberalArtsPromise3 = memoizedFetch("liberalArts", fetchLiberalArts);
...
const SearchDialog = ({ searchInfo, onClose }: Props) => {
  ...
  // 총 페이지 수: filteredLectures의 길이에 따라 계산
  ...
  // 강의의 전공 목록: 중복 제거 처리 (lectures가 변경될 때만 재계산)
